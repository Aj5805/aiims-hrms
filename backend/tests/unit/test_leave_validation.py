...
def test_cl_prefix_weekend_blocked():
    holidays: set[date] = set()
    monday = date(2026, 6, 8)
    rules = {"no_prefix_suffix_weekends": True}
    err = check_prefix_suffix(monday, monday, holidays, rules)
    assert err == "Leave cannot be prefixed to a weekend"


def test_cl_suffix_holiday_blocked():
    wed = date(2026, 6, 10)
    thu = date(2026, 6, 11)
...
def test_no_prefix_suffix_when_disabled():
    d = date(2026, 6, 8)
    assert check_prefix_suffix(d, d, set(), {}) is None
